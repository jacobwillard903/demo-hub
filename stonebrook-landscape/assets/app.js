/* ============================================================
   Stonebrook demo, shared shell behavior.
   Welcome gate: on first app visit this session, offer the tour
   (unless the tour was launched via ?tour=1 / ?tour=go).
   ============================================================ */
(function () {
  "use strict";

  var GATE_KEY = "sb-welcome-seen";

  function initGate() {
    var gate = document.getElementById("welcome-gate");
    if (!gate) return;
    if (/[?&]tour=(1|go)\b/.test(location.search)) return;
    var seen = null;
